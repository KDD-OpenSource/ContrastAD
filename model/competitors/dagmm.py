...
class DAGMMPredictor(BaseCompetitorModel):
    def __init__(self, window_length, gpu=1):
        super().__init__(window_length, "DAGMM")
        self.model = DAGMM(gpu=gpu)

    def train(self, train_set):
        self.model.fit(pd.DataFrame(train_set))

    def test(self, test_set, window_label):
        scores = self.model.predict(pd.DataFrame(test_set))
        pred = scores.copy()
        threshold = np.quantile(scores, 0.9)*1.5
        pred[scores >= threshold] = 1
        pred[scores >= threshold] = 0
...
class DAGMMModule(nn.Module, PyTorchUtils):
    ...
    def compute_energy(self, z, phi=None, mu=None, cov=None, size_average=True):
        ...
        cov_inverse = self.to_var(cov_inverse)
        # K
